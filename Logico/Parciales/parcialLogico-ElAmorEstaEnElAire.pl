...
gustaYDisgusta(pablo, [futbol, basket, comer, bailar, musica], [f1, manualidades, literatura,
historia, debate]).
...
interesesCompletos(Persona) :- generoInteresado(Persona, _), edadInteresada(Persona, _,
_).
gustosYDisgustosCompletos(Persona) :-
datosSuficientes(Persona, gustos, 5),
datosSuficientes(Persona, disgustos, 5).
datosSuficientes(Persona, gustos, Cantidad) :- gustaYDisgusta(Persona, Gustos, _),
cantidadSuficiente(Gustos, Cantidad).
datosSuficientes(Persona, disgustos, Cantidad) :- gustaYDisgusta(Persona, _, Disgustos),
cantidadSuficiente(Disgustos, Cantidad).
cantidadSuficiente(Datos, Cantidad) :- length(Datos, CantidadDeDatos), CantidadDeDatos
>= Cantidad.
persona(Persona) :- persona(Persona, _, _).
genero(Genero) :- persona(_, _, Genero).
% [2]
almaLibre(Persona) :- persona(Persona), forall(genero(Genero),
generoInteresado(Persona, Genero)).
almaLibre(Persona) :- edadInteresada(Persona, EdadMinima, EdadMaxima),
diferenciaEntreEdadesMinima(EdadMaxima, EdadMinima, 30).
...
Enamorado \= Persona.
leInteresaSuGenero(Enamorado, Persona) :- persona(Persona, _, GeneroPersona),
generoInteresado(Enamorado, GeneroPersona).
leInteresaSuEdad(Enamorado, Persona) :-
persona(Persona, EdadPersona, _),
edadInteresada(Enamorado, EdadMinima, EdadMaxima),
between(EdadMinima, EdadMaxima, EdadPersona).
% [5]
hayMatch(UnaPersona, OtraPersona) :- pretendiente(UnaPersona, OtraPersona),
pretendiente(OtraPersona, UnaPersona).
...
leDisgustaAlgoDelOtro(UnaPersona, OtraPersona) :- unGusto(OtraPersona, OtroGusto),
unDisgusto(UnaPersona, OtroGusto).
unGusto(Persona, UnGusto) :- gustaYDisgusta(Persona, Gustos, _), member(UnGusto,
Gustos).
unDisgusto(Persona, UnDisgusto) :- gustaYDisgusta(Persona, _, Disgustos),
member(UnDisgusto, Disgustos).
% [8]
/*
...
promedio(Lista, PromedioFinal) :- sumlist(Lista, Sumatoria), length(Lista, Cantidad),
PromedioFinal is Sumatoria / Cantidad.
% [9]
% ghostea(Ghosteado, Ghosteador).
ghostea(UnaPersona, OtraPersona) :- leEscribio(UnaPersona, OtraPersona),
not(leEscribio(OtraPersona, UnaPersona)).
leEscribio(UnaPersona, OtraPersona) :- indiceDeAmor(UnaPersona, OtraPersona, _). %
Algo redundante, pero se lee más fácil ghostea/2.
%%%% Fin %%%
